...
if(isset($_POST['username']) && isset($_POST['password']) && isset($_POST['passwordRepeated']))
{
	...
	$passwordRepeated = trim($_POST['passwordRepeated']);

	if($frontend->isValidUserName($userName) && $frontend->isValidPassword($password) && $password === $passwordRepeated && $userName != $password)
	{
		$frontend->addNewAccount($userName, $password);
	}else {
